...
export default function PlasticSurgeryPage() {
  ...
  // Get translated clinics based on current locale
  const plasticSurgeryClinics = useMemo(() => {
    return getTranslatedFacilitiesByCategory('plastic-surgery', locale as Locale);
  }, [locale]);

  const filteredClinics = plasticSurgeryClinics.filter(clinic => {
    const matchesSearch = clinic.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         clinic.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         clinic.services.some(s => s.toLowerCase().includes(searchTerm.toLowerCase()));
    const matchesLocation = selectedLocation === 'all' || clinic.location.includes(selectedLocation);
    return matchesSearch && matchesLocation;
  });

  const locations = ['all', 'Gangnam-gu', 'Apgujeong', 'Sinsa-dong', 'Cheongdam-dong'];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight mb-2 text-gray-900 dark:text-white">
            {t.medical.plasticSurgeryClinics}
          </h1>
          <p className="text-base text-gray-600 dark:text-gray-400">
            {t.medical.plasticSurgerySubtitle}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredClinics.map(clinic => (
            <div key={clinic.id} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden shadow-sm hover:shadow-md hover:border-primary-500 dark:hover:border-primary-500 transition-all">
              {/* Clinic Image */}
              <div className="relative w-full h-48 bg-gradient-to-br from-pink-50 to-pink-100 dark:from-gray-800 dark:to-gray-700 overflow-hidden">
                <Image
                  src={clinic.imageUrl}
                  alt={clinic.name}
                  fill
              ...
              <div className="p-6 border-b border-gray-200 dark:border-gray-700">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <h2 className="text-xl font-bold mb-2 text-gray-900 dark:text-white">{clinic.name}</h2>
                    <div className="flex items-center gap-2 mb-2">
                      <div className="flex items-center">
                        <svg className="w-5 h-5 text-yellow-500 fill-current" viewBox="0 0 20 20">
                          <path d="M10 15l-5.878 3.09 1.123-6.545L.489 6.91l6.572-.955L10 0l2.939 5.955 6.572.955-4.756 4.635 1.123 6.545z" />
                        </svg>
                        <span className="ml-1 font-semibold text-gray-900 dark:text-white">{clinic.rating}</span>
                      </div>
                      <span className="text-gray-600 dark:text-gray-400 text-sm">({clinic.reviewCount} {t.medical.reviews})</span>
                    </div>
              ...
              {/* Clinic Body */}
              <div className="p-6">
                <p className="text-gray-700 dark:text-gray-300 mb-4">{clinic.description}</p>

                {/* Services */}
                <div className="mb-4">
                  <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-2">{t.medical.servicesOffered}</h3>
                  <div className="flex flex-wrap gap-2">
                    ...
                    {clinic.services.length > 4 && (
                      <span className="text-primary-600 dark:text-primary-400 px-3 py-1 text-sm font-medium">
                        +{clinic.services.length - 4} {t.medical.more}
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {clinic.languages.map((lang, idx) => (
                      <span key={idx} className="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 px-3 py-1 rounded-lg text-sm flex items-center gap-1">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="mb-4 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                  ...
                  <span className="font-semibold text-gray-900 dark:text-white">{clinic.priceRange}</span>
                </div>

                {/* Book Button */}
                <Link href={`/medical/booking?facilityId=${clinic.id}&category=plastic-surgery`}>
        ...
        {filteredClinics.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 dark:text-gray-400">{t.medical.noClinicResults}</p>
          </div>
        )}
      </div>
    </div>
